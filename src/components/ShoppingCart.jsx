            ...
            {uniqueCartList.map((item) => (
                <div key={item.id} className="cart-item">
                    ...
                    <div>
                        ...
                        <input name="quantity-input" type="number" min="0" max={item.stock} value={cartTracker(item, cartList)} onChange={(e) => handleChange(e, item)}/>
                    </div>
                    <p>${item.price}</p>
                </div>
            ))}
            <p>Subtotal ({cartCounter} items): {total.toFixed(2)}</p>
        </div>
    )
}
